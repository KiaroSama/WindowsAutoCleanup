<#
.SYNOPSIS
    Scheduled-task identity: which task is ours, and the exact command lines the task and the
    installer's elevated relaunch run.
...
.DESCRIPTION
    ...
    in this package acts on a task that Test-WacTaskIsOurs has not proven belongs to this project.

    Everything here only ASKS. The one call that destroys a registration, and the transaction
    boundary that has to hold around it, moved to WindowsAutoCleanup.TaskRemoval.ps1 when this file
    reached the size at which nobody reads it.
