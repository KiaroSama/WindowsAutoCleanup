...
.DESCRIPTION
    ...
    Nothing is registered with the live Task Scheduler. The REAL Remove-WacInstalledTask,
    Resolve-ConflictingTask, Resolve-InterruptedTaskCapture and Restore-CapturedTask run end to end
    against the scheduler _TaskCaptureFixture.ps1 builds - read that file for the mechanism. The
    durable record is real: %ProgramFiles% is redirected into a disposable sandbox and the
    assertions read the file the code actually wrote.
...
. (Join-Path -Path $PSScriptRoot -ChildPath '_TaskCaptureFixture.ps1')
